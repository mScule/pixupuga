<script lang="ts">
    ...
    import { playSounds } from "../stores/SoundSettings";

    let play = false;

    playSounds.subscribe(value => play = value);

    function getSound(type: SoundType): string {
        switch (type) {
            case SoundType.CollectablePointOne:   return SoundCollectablePointOne;
            case SoundType.CollectablePointFive:  return SoundCollectablePointFive;
            case SoundType.CollectableBoxCollect: return SoundCollectableBoxCollect;
            case SoundType.CollectableBoxPlace:   return SoundCollectableBoxPlace;

            case SoundType.HigherBoulder:     return SoundHigherBoulder;
            case SoundType.HigherBox:         return SoundHigherBox;
            case SoundType.HigherDestruction: return SoundHigherDestruction;
            case SoundType.HigherHit:         return SoundHigherHit;

            case SoundType.LevelLose: return SoundLevelLose;
            case SoundType.LevelWin:  return SoundLevelWin;

            case SoundType.LowerTrapSpikesUp:   return SoundLowerTrapSpikesUp;
            case SoundType.LowerTrapSpikesDown: return SoundLowerTrapSpikesDown;

            case SoundType.LowerHit: return SoundLowerHit;
        }
    }
